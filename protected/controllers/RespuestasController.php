...
class RespuestasController extends Controller
{
	...
	public function actionCreate()
	{
		...
		$contacto=Contacto::model()->load($user->nick);
		if(empty($contacto)){
			$this->redirect(array('mensajefinal'));
		}
		$model->id_con=$contacto->id_con;
	...
	/**
	 * Controlador para router
	*/
	public function actionRouter(){
		$user=User::model()->getUserActualModel();
		$contacto=Contacto::model()->load($user->nick);
		$contactoReal=Contacto::model()->load4($user->nick);
		if(empty($contacto)){
			if($contactoReal->empresa=='Promigas'){
				$comunicaciones=Comunicaciones::model()->load($user->nick);
				if(empty($comunicaciones)){
					$this->redirect(array('comunicaciones/create'));
				}
			}
			$this->redirect(array('mensajefinal'));
		}
		$this->redirect(array('Batchcreate'));
	}

	/**
	 * Renderizar mensaje final
	*/
	public function actionMensajeFinal(){
		$this->render('msgFinal');
	}
}
